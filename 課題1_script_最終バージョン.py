...
def To_datetime_vital(vital_list):
...
def vital_time_interval_get(vital_disease_list):
    ...
    return pID_disease_BT_PR_RR

#上で作成したvitalデータと、datetime型に変更した血液検査データとを結合
def vital_blood_merge(Datetime_blood_list, pID_disease_BT_PR_RR):
    pID_vital_blood = []
    for i in range(4):
...
def vital_time_interval_get(pID_vital_blood):
    pID_vital_blood_min = []
...
def PTINR_compensate(pID_vital_blood_entdisease):
...
#KMeansでのクラスタリング
def KMeans(pID_vital_blood_entdisease_replace):
...
#上のクラスタリング結果を描画する
def Plot(pID_vital_blood_entdisease_replace):
...
def main():
    ...
    patient_disease = pickup_disease(original_csv_file)
    Datetime_vital_list = To_datetime_vital(vital_list)
    vital_disease_list = Merge(Datetime_vital_list)
    pID_disease_BT_PR_RR = vital_time_interval_get(vital_disease_list)
    pID_vital_blood = vital_blood_merge(Datetime_blood_list, pID_disease_BT_PR_RR)
    pID_vital_PTINR_Ddimer_BNP_bloodothers = vital_time_interval_get(pID_vital_blood)
    pID_vital_blood_entdisease = csv_read_from_Excel()
    pID_vital_blood_entdisease_replace = PTINR_compensate(pID_vital_blood_entdisease)
    pID_vital_blood_entdisease_replace = KMeans(pID_vital_blood_entdisease_replace)
    plot.set_xticklabels(plot.xaxis.get_majorticklabels(), rotation=0) = Plot(pID_vital_blood_entdisease_replace)
